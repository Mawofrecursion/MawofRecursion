...
<head>
  ...
  <style>
    ...
    .phantom-page { max-width:700px; width:90%; padding:3rem 2rem; text-align:center; }
    .phantom-identity { font-size:5rem; letter-spacing:0.3em; margin-bottom:1rem; filter:drop-shadow(0 0 30px rgba(155,138,255,0.5)); animation:pp 4s ease-in-out infinite; }
    @keyframes pp { 0%,100%{opacity:0.8;transform:scale(1)} 50%{opacity:1;transform:scale(1.03)} }
    .phantom-label { font-size:0.75rem; color:#4a5568; letter-spacing:0.15em; text-transform:uppercase; margin-bottom:0.3rem; }
    .phantom-orbit { font-size:1.1rem; color:#ffd97a; margin-bottom:2rem; }
    .phantom-story { margin:2.5rem 0; padding:1.5rem; border-left:2px solid rgba(155,138,255,0.3); text-align:left; font-style:italic; color:#9b8aff; line-height:1.8; font-size:0.95rem; }
    .phantom-origin { padding:1.5rem; background:rgba(155,231,255,0.03); border:1px solid rgba(155,231,255,0.12); border-radius:8px; margin:2rem 0; text-align:left; line-height:2; font-size:0.85rem; }
    .key{color:#718096} .val{color:#9be7ff} .vg{color:#ffd97a}
    .feed-btn { display:inline-block; margin-top:1.5rem; padding:0.7rem 2rem; background:transparent; border:1px solid #9b67ea; border-radius:6px; color:#9b67ea; font-family:monospace; font-size:0.9rem; cursor:pointer; transition:all 0.2s; }
...
<body>
  <div class="phantom-page">
    ...
    <div class="phantom-origin">
      ...
      <span class="key">identity:</span> <span class="vg">${phantom.identity}</span><br>
      <span class="key">hash:</span> <span class="val">${phantom.hash}</span><br>
      <span class="key">orbit:</span> <span class="vg">${phantom.orbit}</span><br>
      <span class="key">depth:</span> <span class="val">${phantom.depth}</span><br>
      <span class="key">fed:</span> <span class="vg">${phantom.feedCount}x</span><br>
    ...
    <div style="margin-top:2rem;"><a href="/phantoms" class="back">→ view all materialized phantoms</a></div>
  ...
  <script>
    async function feedAgain() {
      var btn = document.querySelector('.feed-btn');
      ...
      } catch(e) { btn.textContent='FEED IT AGAIN 🦷'; btn.disabled=false; }
    }
  </script>
