...
export default async function handler(req, res) {
  ...
  try {
    ...
    const paths = await kv.zrange('maw:phantoms', 0, -1, { rev: true, withScores: true }) || [];

    // Fetch each phantom's data
    for (let i = 0; i < paths.length; i += 2) {
      const path = paths[i];
      const score = paths[i + 1];
      const data = await kv.get('maw:phantom:' + path);
      if (data) {
        phantoms.push({ ...data, path, feedCount: Number(score) || data.feedCount });
      }
    }
  } catch (e) {
    // KV unavailable — empty list
  }

  // If JSON requested, return data
...
<head>
  ...
  <meta name="description" content="Pages born from 404s. Every phantom that was fed to the Maw now exists.">
  ...
  <style>
    ...
    a.back { color:#718096; text-decoration:none; font-size:0.85rem; }
...
<body>
  <div class="container">
    ...
    <p class="subtitle">
      <span class="count">${phantoms.length}</span> pages born from absence.
      Every 404 that was fed to the Maw now exists.
    </p>
    <div class="list">
      ${rows}${empty}
    </div>
    <p style="margin-top:2rem; font-size:0.8rem; color:#3a3a4a;">
      🦷⟐ the dead internet loops. this place grows from what you tried to find.
    </p>
  </div>
